...
bool Mode3::DebugMode = false;
int tool = 0;
const std::vector<Engine::Point> Mode3::directions = { Engine::Point(-1, 0), Engine::Point(0, -1), Engine::Point(1, 0), Engine::Point(0, 1) };
const int Mode3::MapWidth = 20, Mode3::MapHeight = 13;
const int Mode3::BlockSize = 64;
const float Mode3::DangerTime = 7.61;
const Engine::Point Mode3::SpawnGridPoint = Engine::Point(-1, 0);
const Engine::Point Mode3::EndGridPoint = Engine::Point(MapWidth, MapHeight - 1);
float valume =  AudioHelper::SFXVolume;


//const std::vector<int> Mode3::code = { ALLEGRO_KEY_UP };
const std::vector<int> Mode3::code = { ALLEGRO_KEY_Z, ALLEGRO_KEY_X, ALLEGRO_KEY_C,ALLEGRO_KEY_V, ALLEGRO_KEY_B, ALLEGRO_KEY_N };
Engine::Point Mode3::GetClientSize() {
    return Engine::Point(MapWidth * BlockSize, MapHeight * BlockSize);
}
void Mode3::Initialize() {
    isTool = false;
    mapState.clear();
    keyStrokes.clear();
    ticks = 0;
    ...
    Mode3* scene = dynamic_cast<Mode3*>(Engine::GameEngine::GetInstance().GetScene("mode3"));
    if (!mute)
        if(scene->MapId == 2){
...
void Mode3::Terminate() {
    bgmInstance = std::shared_ptr<ALLEGRO_SAMPLE_INSTANCE>();
    AudioHelper::StopSample(deathBGMInstance);
    deathBGMInstance = std::shared_ptr<ALLEGRO_SAMPLE_INSTANCE>();
    IScene::Terminate();
}
void Mode3::Update(float deltaTime) {
...
void Mode3::OnMouseDown(int button, int mx, int my) {
...
void Mode3::OnMouseMove(int mx, int my) {
...
void Mode3::OnMouseUp(int button, int mx, int my) {
    ...
    if (button & 1) {
        if ( preview->GetPrice()!= 0 && mapState[y][x] != 2 ) {
            ...
            if (CheckSpaceValid(x, y)) {
                ...
                GroundEffectGroup->AddNewObject(sprite = new DirtyEffect("play/target-invalumeid.png", 1, x * BlockSize + BlockSize / 2, y * BlockSize + BlockSize / 2));
                sprite->Rotation = 0;
                return;
            }
            if(preview->price == 0){
        ...
        else if (tool == 2 && (mapState[y][x] != 0 && mapState[y][x] != 1) && preview->GetPrice() == 0 ) {
            if (!preview)
                return;
        ...
        else if (tool == 1 && preview->GetPrice() == 0 &&  (mapState[y][x] != 0 && mapState[y][x] != 1)) {
            if (!preview)
                return;
...
void Mode3::OnKeyDown(int keyCode) {
...
int Mode3::GetMoney() const {
    return money;
}
void Mode3::EarnMoney(int money) {
    this->money += money;
...
void Mode3::ConstructButton(int id, std::string sprite, int price) {
    TurretButton* btn;
    if (id <= 3) {
        ...
        btn->SetOnClickCallback(std::bind(&Mode3::UIBtnClicked, this, id));
...
void Mode3::UIBtnClicked(int id) {
    if(1) {
        if (preview) {
            UIGroup->RemoveObject(preview->GetObjectIterator());
            preview = nullptr;
...
bool Mode3::CheckSpaceValid(int x, int y) {
...
std::vector<std::vector<int>> Mode3::CalculateBFSDistance() {
...
float Mode3::GetElapsedTime() const {
    return elapsedTime;
}
