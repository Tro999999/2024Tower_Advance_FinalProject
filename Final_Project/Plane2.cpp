...
MODE2* Plane2::getMode2Scene() {
    return dynamic_cast<MODE2*>(Engine::GameEngine::GetInstance().GetActiveScene());
}
Plane2::Plane2() : Sprite("play/ironman.png", -100, Engine::GameEngine::GetInstance().GetScreenHeight() / 2), stage(0), timeTicks(0) {
    for (int i = 1; i <= 10; i++) {
...
void Plane2::Update(float deltaTime) {
    int phase;
    float scaleExp;
    switch (stage) {
        case 0:
            // Check if out of boundary.
            if (!Engine::Collider::IsRectOverlap(Position - Size / 2, Position + Size / 2, Engine::Point(-100, 0), MODE2::GetClientSize())) {
                Position = MODE2::GetClientSize() / 2;
