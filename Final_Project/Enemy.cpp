...
void Enemy::Hit(float damage) {
	...
	if (hp <= 0) {
        ...
        //getPlayScene()->EnemyGroup->AddNewObject(new PoopEnemy(SpawnCoordinate.x, SpawnCoordinate.y));
        //getPlayScene()->EnemyGroup->AddNewObject(new PoopEnemy(30, 30));
    }
...
void Enemy::Update(float deltaTime) {
    if (money == 500) {
        ...
        if(gentime >= 1){
            gentime = 0;
            Enemy* enemy;
            getPlayScene()->EnemyGroup->AddNewObject(enemy = new DevilEnemy(Position.x, Position.y));
...
void Enemy::IsBoss(){
    if(speed == 20){
        getPlayScene()->EnemyGroup->AddNewObject(new PoopEnemy(-32, 32));
    }
}
